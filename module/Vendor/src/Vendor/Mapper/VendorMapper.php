...
//            return $resultSet->initialize($result);
	...
	public function findByEmail($data)
    {
		...
		$select = $sql->select();
		$select->columns( array(
		'emailId' => 'email_id','password' => 'password','first_name' => 'first_name','last_name' => 'last_name'));
		$select->from('users');
		$select->where($where);
		$statement = $sql->prepareStatementForSqlObject($select);
		...
		$dbemail_id = $rows[$key]['emailId'];
		
		$dbPassword = $rows[$key]['password'];}
		//print_r($rows); exit;
		if ($email_id != $dbemail_id)
		{
